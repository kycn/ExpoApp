...
import com.kycn.expoapp.characters.view.CharacterItemView
    ...
    fun <T : BaseView> newInstance(viewClass: KClass<T>, @Nullable container: ViewGroup?): T {
        ...
        when (viewClass) {
            CharactersView::class -> {
                view = CharactersView(layoutInflater, this, container)
            }
            CharacterItemView::class -> {
                view = CharacterItemView(layoutInflater, imageLoader, container)
            }
            else -> {
